...
object BackendServer {
  ...
  private val docsEpt: List[ServerEndpoint[Any, IO]] =
    SwaggerInterpreter()
      .fromEndpoints[IO](NodeRts.docEpt ++ UserRts.docEpt ++ ConnRts.docEpt, "BackendServer", "1.0") // On "/docs"
  ...
  // FrontEnd routes
  private val backEndOriginRts: HttpRoutes[IO] = HttpRoutes.of[IO] { case GET -> Root / "datapiu-backend-origin" =>
    Ok(ConfigLoader.backEndOrigin)
  }
  ...
  private val frontEndRts: HttpRoutes[IO]      =
    backEndOriginRts <+> docsRts <+> staticFilesRts <+> indexHTMLRts // Highest routes priority defined first
  ...
  def run: IO[Unit] = for {
    ...
    _ <- EmberServerBuilder
           ...
           .withHttpApp(CORS.policy.withAllowOriginAll(frontEndRts).orNotFound)
           .build
           .use(_ => IO.never)
           .start
